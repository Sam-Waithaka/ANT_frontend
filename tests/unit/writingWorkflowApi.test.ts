...
import { createWorkflowNote, createWritingMediaEmbed, returnWritingToDraft, scheduleWriting, submitWritingForReview, unscheduleWriting } from '../../src/services/writingApi';
...
describe('writing workflow API', () => {
  ...
  it('uses the signed-off workflow endpoints and bodies', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(jsonResponse({ id: 5, note: 'Editorial note.', writing: 2 }));
    ...
    await createWorkflowNote('token', { writing: 2, note: 'Editorial note.', action: 'REVIEW' });
    ...
    expect(String(fetchMock.mock.calls[2][1]?.body)).not.toContain('scheduled_publish_at');
    ...
    expect(fetchMock.mock.calls[5]).toEqual(['/v1/writing-workflow-notes/', expect.objectContaining({
      body: JSON.stringify({ writing: 2, note: 'Editorial note.', action: 'REVIEW' }), method: 'POST',
    })]);
    expect(String(fetchMock.mock.calls[5][1]?.body)).not.toContain('content_json');
  });
});
