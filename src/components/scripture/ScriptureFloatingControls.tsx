...
const menuBase =
  'absolute bottom-full mb-3 rounded-[1.5rem] border p-3 shadow-2xl backdrop-blur-xl';
...
const ScriptureFloatingControls = ({
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-[4.75rem] z-40 px-4 md:bottom-6 md:left-44 md:right-0 md:px-6 lg:left-[23rem] xl:right-80">
      <div className={`pointer-events-auto mx-auto flex w-fit max-w-full items-center gap-2 rounded-[2rem] border p-2 shadow-2xl backdrop-blur-xl ${surfaceClass}`}>
        <button
          type="button"
          onClick={onPrevious}
          ...
          className={`grid size-11 shrink-0 place-items-center rounded-full transition disabled:cursor-not-allowed disabled:opacity-35 ${
            darkMode ? 'bg-white/10 text-stone-100 hover:bg-white/15' : 'bg-zinc-100 text-zinc-950 hover:bg-zinc-200'
          }`}
        ...
        <div className="flex min-w-0 flex-wrap items-center justify-center gap-2">
          <div className="relative">
            ...
            {openMenu === 'version' && (
              <div className={`${menuBase} ${surfaceClass} left-0 w-72`}>
                ...
                <div className="grid max-h-72 gap-1 overflow-y-auto">
                  {versions.map((version) => (
                    ...
                    >
                      ...
                      <span className={`ml-2 text-xs ${darkMode ? 'text-stone-400' : 'text-zinc-500'}`}>{version.name}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="relative">
            ...
            {openMenu === 'chapter' && (
              <div className={`${menuBase} ${surfaceClass} right-0 w-72 sm:w-80`}>
