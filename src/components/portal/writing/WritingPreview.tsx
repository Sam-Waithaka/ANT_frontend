...
const WritingPreview = ({ contentJson, coverImage, darkMode, excerpt, mediaEmbeds, onCoverImageRefresh, title }: WritingPreviewProps) => {
  ...
  return (
    <article aria-label="Article preview" className={'overflow-hidden rounded-3xl border shadow-lg ' + surfaceClass}>
      <ResponsiveImage alt={coverImage?.alt_text || coverImage?.title || 'Article cover image'} asset={coverImage} className="aspect-[16/8] w-full object-cover" fetchPriority="high" loading="eager" onRefreshAsset={onCoverImageRefresh} preset="articleCover" />
      <div className="mx-auto max-w-3xl px-6 py-8 sm:px-10 sm:py-12">
        ...
        <div className={'mt-8 border-t pt-8 ' + (darkMode ? 'border-white/10' : 'border-black/10')}>
          <WritingContentRenderer contentJson={contentJson} darkMode={darkMode} emptyMessage="Begin writing to see the article preview." mediaEmbeds={mediaEmbeds} />
        </div>
      </div>
    </article>
  );
