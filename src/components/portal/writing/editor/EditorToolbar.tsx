        ...
        <div className="min-h-0 overflow-y-auto px-6 py-5 sm:px-7">
          <div className="grid gap-5">
            ...
            <section className="grid gap-2">
              ...
              {linkDialog.selectedText ? <div className={darkMode ? 'flex items-start gap-4 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-4' : 'flex items-start gap-4 rounded-2xl border border-[#eaded0] bg-[#fffaf0] px-4 py-4'}><span className={darkMode ? 'grid size-10 shrink-0 place-items-center rounded-full bg-white/5 text-stone-300' : 'grid size-10 shrink-0 place-items-center rounded-full bg-white text-zinc-500'}><Quote size={18} /></span><p><span className="block text-sm font-black">{linkDialog.selectedText}</span><span className={darkMode ? 'mt-1 block text-xs text-stone-400' : 'mt-1 block text-xs text-zinc-500'}>This is the text that will be linked.</span></p></div> : <label className="grid gap-2"><input className={darkMode ? 'w-full rounded-2xl border border-white/10 bg-[#0d0d0d] px-4 py-3 text-sm text-stone-100 outline-none transition placeholder:text-stone-500 focus:border-red-700 focus:ring-4 focus:ring-red-900/20' : 'w-full rounded-2xl border border-[#eaded0] bg-[#fffaf0] px-4 py-3 text-sm text-zinc-950 outline-none transition placeholder:text-zinc-400 focus:border-red-700 focus:ring-4 focus:ring-red-100'} onChange={(event) => setLinkDialog({ ...linkDialog, error: '', text: event.target.value })} placeholder="Text to display" value={linkDialog.text} /></label>}
            </section>
            <section className="flex items-center justify-between gap-5">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.22em] text-red-800 dark:text-red-200">Open in new tab</p>
                <p className={darkMode ? 'mt-1 text-xs text-stone-400' : 'mt-1 text-xs text-zinc-500'}>Readers will not lose their place while exploring another resource.</p>
              </div>
              <button aria-checked={linkDialog.newTab} className={'relative h-8 w-14 shrink-0 rounded-full border transition ' + (linkDialog.newTab ? 'border-red-800 bg-red-800' : darkMode ? 'border-white/10 bg-white/10' : 'border-black/10 bg-zinc-200')} onClick={() => setLinkDialog({ ...linkDialog, newTab: !linkDialog.newTab })} role="switch" type="button"><span className={'absolute top-1 size-6 rounded-full bg-white shadow transition ' + (linkDialog.newTab ? 'left-7' : 'left-1')} /></button>
            </section>
