...
type ArticleEditorProps = { contentJson?: unknown; darkMode: boolean; editable?: boolean; mediaEmbeds?: WritingMediaEmbedLike[]; mediaDisabledLabel?: string; onChange?: (contentJson: LexicalContentJson, plainText: string) => void; onImageBlocksChange?: (blocks: ImageBlockMetadata[]) => void; onPendingMediaInserted?: () => void; onRequestMedia?: () => void; pendingMediaEmbed?: PendingMediaEmbed; placeholder?: string; saveState?: EditorSaveState; };
...
const ImageBlockDragPlugin = () => {
  ...
  useEffect(() => editor.registerRootListener((rootElement, previousRootElement) => {
    ...
    const onDragOver = (event: DragEvent) => { if (event.dataTransfer?.types.includes('application/x-aic-writing-image')) event.preventDefault(); };
    const onDrop = (event: DragEvent) => { const sourceKey = event.dataTransfer?.getData('application/x-aic-writing-image'); if (!sourceKey) return; event.preventDefault(); const targetElement = (event.target as HTMLElement | null)?.closest('[data-lexical-node-key]') as HTMLElement | null; const targetKey = targetElement?.getAttribute('data-lexical-node-key'); editor.update(() => { const source = $getNodeByKey(sourceKey); if (!source) return; const sourceTop = source.getTopLevelElementOrThrow(); const target = targetKey ? $getNodeByKey(targetKey) : null; const targetTop = target?.getTopLevelElementOrThrow(); if (!targetTop || targetTop.is(sourceTop)) { $getRoot().append(sourceTop); return; } const rect = targetElement?.getBoundingClientRect(); if (rect && event.clientY > rect.top + rect.height / 2) targetTop.insertAfter(sourceTop); else targetTop.insertBefore(sourceTop); }); };
