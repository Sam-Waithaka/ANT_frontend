...
const WritingSeriesAdminPage = () => {
  ...
  const searchWritings = async (event?: FormEvent) => {
    ...
    try {
      ...
      setMessage(
        err instanceof Error ? err.message : "Unable to search writings.",
      );
    } finally {
      setSearching(false);
    }
  ...
  const addWriting = async (writingId: number | string) => {
    ...
    try {
      await createWritingSeriesItem(auth.accessToken, {
        order: sortSeriesItems(series.items).length,
        series: series.id,
        writing: writingId,
      });
      setQuery("");
  ...
  const reorderItems = async (
    ...
    try {
      ...
      setMessage(
        err instanceof Error ? err.message : "Unable to reorder series items.",
      );
      loadSeries();
    }
  };
  ...
  return (
    <WritingStudioShell>
      <div className="grid gap-6">
        <Link className={actionButtonClass} to="/portal/writing/library">
          Back to Library
        </Link>
        <section
          className={`rounded-[2rem] border p-6 shadow-lg ${portalSurface.panel(darkMode)}`}
        >
          ...
          <div className="mt-4 flex flex-wrap items-start gap-5">
            {coverUrl ? (
              <img
                alt=""
                className="h-28 w-40 rounded-3xl border border-[#eaded0] object-cover dark:border-white/10"
                src={coverUrl}
              />
            ) : null}
            <div className="min-w-0 flex-1">
              <h1 className="font-serif text-4xl leading-tight sm:text-5xl">
                {loading ? "Loading series..." : seriesName(series)}
              </h1>
              {series?.slug ? (
                <p
                  className={`mt-2 break-all text-sm ${portalSurface.softMutedText(darkMode)}`}
                >
                  /{series.slug}
                </p>
              ) : null}
              {series?.description ? (
                <p
                  className={`mt-4 max-w-3xl text-sm leading-7 ${portalSurface.mutedText(darkMode)}`}
                >
                  {series.description}
                </p>
              ) : null}
              <div className="mt-4 flex flex-wrap gap-2">
                {series?.is_active ? (
                  <span className={statusBadgeClass("PUBLISHED")}>Active</span>
                ) : (
                  <span className={statusBadgeClass("ARCHIVED")}>Inactive</span>
                )}
                {series?.is_featured ? (
                  <span className={statusBadgeClass("DRAFT")}>Featured</span>
                ) : null}
                {series?.slug ? (
                  <Link
                    className={actionButtonClass}
                    target="_blank"
                    to={`/resources?series_slug=${encodeURIComponent(series.slug)}`}
                  >
                    Public preview
                  </Link>
                ) : null}
          ...
          {message ? (
            <p className="mt-5 text-sm font-bold text-red-800">{message}</p>
          ) : null}
        </section>
        ...
        >
          ...
          <form
            className="mt-3 grid gap-3 md:grid-cols-[1fr_14rem_auto]"
            onSubmit={(event) => void searchWritings(event)}
          >
            <input
            ...
            <select
              className={inputClass}
              onChange={(event) =>
                setStatus(event.target.value as WritingStatus | "ALL")
              }
              value={status}
            >
            ...
            <button
              className="rounded-full bg-red-800 px-5 py-3 text-sm font-black text-white disabled:opacity-50"
              disabled={searching}
              type="submit"
            >
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <button
                        ...
                        onClick={() => void addWriting(writing.id)}
                        type="button"
                      >
        ...
        >
          ...
          <p className={helperClass}>
            Drag items, or use Move up / Move down. Ordering is stored through
            the dedicated series reorder endpoint.
          </p>
          <ol className="mt-4 grid gap-3">
            {orderedItems.length ? (
                ...
                <li
                  ...
                  className={
                    darkMode
                      ...
                      : "rounded-2xl border border-[#eaded0] bg-white p-4"
                  }
                  draggable
                  ...
                  onDrop={() => void dropItem(orderedItems, index)}
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        disabled={index === 0}
                        onClick={() =>
                          void reorderItems(orderedItems, index, index - 1)
                        }
                        type="button"
                      >
                        Move up
                      ...
                      <Link
                        className={actionButtonClass}
                        to={`/portal/writing/${item.writing}`}
                      >
                        Open
                      </Link>
                      <button
              ...
              <p className={`text-sm ${portalSurface.softMutedText(darkMode)}`}>
